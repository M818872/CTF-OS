...
from app.schemas.investigation import ActivityRead, ExecuteRequest, InvestigationCreate, InvestigationRead, PlanRequest, WorkspaceRead
...
async def list_investigations(session: AsyncSession = Depends(get_session)) -> list[Investigation]:
...
async def create_investigation(payload: InvestigationCreate, session: AsyncSession = Depends(get_session)) -> Investigation:
...
async def get_investigation(investigation_id: UUID, session: AsyncSession = Depends(get_session)) -> Investigation:
...
async def get_workspace(investigation_id: UUID, session: AsyncSession = Depends(get_session)) -> WorkspaceRead:
...
async def plan_investigation(investigation_id: UUID, payload: PlanRequest, session: AsyncSession = Depends(get_session)) -> WorkspaceRead:
    ...
    existing = await session.scalar(select(InvestigationActivity.id).where(InvestigationActivity.investigation_id == investigation_id, InvestigationActivity.kind == "plan"))
    if existing is None:
        session.add(InvestigationActivity(investigation_id=investigation_id, kind="plan", action="Create investigation plan", status="completed", details="Route: " + ", ".join(task.specialist for task in tasks)))
        for task in tasks:
            session.add(InvestigationActivity(investigation_id=investigation_id, kind="capability", action=f"Prepare {task.specialist} specialist", status="ready", details=", ".join(task.capabilities)))
...
async def execute_capability(investigation_id: UUID, payload: ExecuteRequest, session: AsyncSession = Depends(get_session)) -> WorkspaceRead:
    investigation = await _get_investigation(investigation_id, session)
    ...
    session.add(InvestigationActivity(investigation_id=investigation_id, kind="execution", action=f"Run capability {capability}", status="queued", details=payload.input_text.strip() or "No input supplied"))
...
async def get_report(investigation_id: UUID, session: AsyncSession = Depends(get_session)) -> str:
    investigation = await _get_investigation(investigation_id, session)
    activities = await session.scalars(select(InvestigationActivity).where(InvestigationActivity.investigation_id == investigation_id).order_by(InvestigationActivity.created_at.asc()))
    lines = [f"# CTF-OS Investigation: {investigation.title}", "", f"Status: {investigation.status}", f"Challenge type: {investigation.challenge_type}", "", "## Timeline"]
    events = list(activities.all())
    lines.extend(f"- {event.created_at.isoformat()} — {event.action} [{event.status}]" + (f" — {event.details}" if event.details else "") for event in events)
    if not events:
